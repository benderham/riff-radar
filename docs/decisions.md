...
What is lost is stated rather than discovered: no reissue or remaster detection, no EP track and duration thresholds, and the genre, personnel and label terms drop out of ranking. (The label only half-drops, and the wording above was corrected on 17 September — see the second amendment.) `runs` gains `musicbrainz_degraded`, and the shortlist items say so. The run still completes and still writes, because a degraded shortlist Ben reviews is worth more than a dead run, and Ben reviews everything.

No new termination reason: degradation is a property of a run, not a reason it stopped.

**Implemented on 17 September 2026.** Four things the decision did not spell out and the code had to settle. A degraded lookup records the failure category `unavailable` — which is what that category was defined for, a provider this run has stopped knocking at (ADR-0048) — and that is what makes the counter safe to derive: the trailing run of silent lookups keeps growing while the run is degraded, so a resume recomputes the same verdict instead of finding a run of zero and cheerfully asking again. The verdict handed back at lookup time is degraded-aware too, so the model is told a release is eligible on its source's word at the moment it asks, rather than discovering it at `finish` (ADR-0053's lesson, applied here for free). Notion states it in the `Rationale` property rather than in a column of its own: a new property would have to exist in Ben's database before the preflight would let a run start, and the point of degradation is that the run finishes. And the stamp that carries it — `judgedWithoutMusicbrainz` — sits on `ShortlistItem` *outside* the action schema, set by the run and never by the model, so saying it costs no `ACTION_SCHEMA_VERSION` bump and therefore invalidates no unfinished run. It is stamped only on items with no release-group id, because a release looked up before the outage was judged on MusicBrainz's word like any other: degradation is a property of the run, but the evidence is per release.

`TracedStep` gained `tool_name` and `failure_category`, which the replay needs and a resume reads; both columns already existed. Only *dispatched* lookups are counted: `invalid_action` records the tool the model named, so a malformed `lookup_release` proposal would otherwise read as an answered lookup and set a degraded run knocking again. And an aborted run carries the flag as a finished one does — unlike the two columns `abortRun` deliberately leaves unset (ADR-0047), a killed run's own steps say outright whether it had given up, and a count of degraded runs that missed the killed ones would be wrong about exactly the runs this exists for.

**Amended 17 September 2026 — Ben's decision, on the evidence of run `e480b9c1`.** The first degraded run from Ben's own machine worked exactly as specified and proposed nothing: it ended `no_candidates` holding an empty shortlist, having refused all fourteen of its candidates. The reason is in its trace — `MusicBrainz was unavailable, and no source stated a format`, thirteen times. Neither Loudwire's calendar nor Wikipedia states a format for most entries, so the stated-format fallback had nothing to read.

That is ADR-0045's rule meeting a case it was not written for. There, "no stated format still excludes" is a safe conservative default *because MusicBrainz normally answers*, and the release with no type is the rare one. Under degradation the rare case is every case, so the same default stops being conservative and becomes a blanket refusal — reaching, by a different route, exactly the dead run this decision exists to prevent.

So while degraded, **silence is the calendar's word rather than missing data**: a release no source described is eligible, because being listed in a new-release calendar inside the window is itself a claim that it is new work, and the date rule still has to agree. A source that *does* describe it is still believed in both directions — a stated album passes, a stated live album, EP, single, compilation or reissue is refused, and a format nobody recognises is refused. Nothing changes outside degradation: an unverified release, where MusicBrainz answered *about this release* and had never heard of it, still needs a stated format, and so does an untyped release group.

**Corrected 17 September 2026, on the evidence of run `3e657aa3`.** "Ranking loses its label term" was never quite true and is worth stating exactly, because it is the last signal a degraded run has. `scoreRelease` already falls back to the label a *source* printed, so the term can still match — but the match is containment in one direction, term inside value, and Loudwire prints `Reigning Phoenix` where the profile says `Reigning Phoenix Music`. The signal therefore missed, silently, on a label that really is on the include list: the run's five shortlisted releases all scored zero and were ordered by the model's own preference through the tie-break. Undegraded runs are insulated from this because MusicBrainz supplies the label in full. Whether label matching should be symmetric is a ranking decision and is not taken here.

**Consequences:** A degraded run can propose a reissue or a live album that a source mislabelled, with nothing to catch it — the same narrow exposure ADR-0045 accepted, now reachable by an outage rather than by a missing field, and widened by the amendment above: a reissue nobody described at all now reaches the shortlist too. Run `e480b9c1` held two of them, *The Last Stand 10th Anniversary Edition* and *The Damn Truth Re-Release*, and both say what they are in their titles — which is why the degraded lookup now tells the model to read the title, and why it matters that Ben reviews every proposal. Two consecutive failures is a low bar and a single flaky minute will trip it; the alternative is a run that spends its ceiling discovering the same fact more slowly. Degraded runs must be excluded from, or labelled in, milestone 3's evaluation, or they will look like a ranking regression.
...
**Implemented on 17 September 2026.** Three things the decision did not spell out and the code had to settle. A refused finish is recorded as a `finish` step carrying the validator's errors in `error`, and that is all the trace needed: a refusal is a `finish` with an error, a run that ended is a `finish` without one, so whether the repair is spent is derivable from the recorded steps with no new step kind and no new column — which matters because a new kind would be a schema migration, and the counter would otherwise have to be stored in the one place ADR-0046 reserves for what cannot be derived. The message the model is handed is deliberately *not* `refusalMessage`: that one says the action was not dispatched, and a refused `finish` was — what was rejected is the shortlist, not the call, and the model is being asked for a corrected list rather than a different action. And because the repair is spent on a step the loop counts like any other, the step and cost ceilings bound it for free: a resume with one step left records its refusal and then ends at `max_steps_exceeded` rather than buying a thirty-first call.

Two edges the reviews found, both settled here rather than left implicit. A trace can hold *two* refused finishes — the repair and the second refusal that ended the run — because both record the validator's errors in the same column; only the first of them ever said anything to the model, so the replay hands back the errors for the first refusal by position and treats a later one as what it was, the end of a run. And where this decision meets ADR-0025 the quiet week wins: a model that answers the repair by dropping every item ends `no_candidates`, not `validation_failed`, because dropping items is exactly what the repair is for and an empty shortlist is a real answer. So `validation_failed` means the model was told twice and still named something ineligible; it does not mean it was told twice and gave up.

Four existing tests had to change, and the change is worth stating because it is the behaviour this decision alters rather than a weakened assertion. Each scripted a `finish` whose shortlist the validator silently refused — an unverified release with no stated format — and relied on that refusal ending the run in one step while testing something else entirely: step durations, the stable prefix, token accounting, and two runs sharing a database. A refusal now buys a repair, so each of them grew a step. They end on an empty shortlist instead, which is the quiet week of ADR-0025: one step, no write, and it never reaches the validator, so their arithmetic is unchanged and their subject is not.
...
## ADR-0068: `profile_version` is joined from the store and the committed traces, and `unknown` is a bucket

**Status:** ACCEPTED — 21 September 2026, while building ticket 07's taste command.

ADR-0060 requires Acceptance Rate, Taste Yield and Shortlist Fill Rate to be reported **per `profile_version`**, and `profile_version` is not in Notion: `NOTION_PROPERTIES` carries a `Run ID` and nothing about the profile. So the version has to be joined on locally, run by run — and the local record is incomplete. The milestone-1 database was deleted at milestone 2's schema change, which was the right call at the time and has the consequence nobody noticed until a metric needed it: of the four runs that have ever written to this Notion database, `riff-radar.db` remembers one.

The join therefore reads **the store and the trace exports under `docs/evidence/`**, which recover two more, and a proposal whose run neither names is reported in an **`unknown`** bucket rather than pooled into a real version or dropped. Its three numbers are readable; only its comparison to another profile is not. Pooling would attribute proposals to a profile that did not produce them, and dropping would silently shrink every `n` — the two failures the per-version split exists to prevent.

The **Fill Rate denominator** is the runs that reached a shortlist decision: `WRITE_PERMITTED` plus `no_candidates`, or any run that had already written a proposal. A run that proposed nothing is in the denominator, because proposing nothing is the extreme of the caution the metric exists to expose; a run that was killed or spent its steps is not, because scoring it as a short shortlist blames the agent for a laptop lid.

**Consequences:** the taste numbers are only as complete as this machine's records, and that is stated in the output rather than hidden — a second machine reading the same Notion database gets a different split, with the same total. The alternative was writing the profile version into Notion as a tenth property, which was refused for the reason ADR-0063 refuses a fourth meaning on a row: the database is Ben's, the version is the agent's, and a property added for a metric is a schema change to somebody else's workspace. Nothing recovers run `530ebea6`, whose five proposals predate every surviving record; it is the permanent floor of the `unknown` bucket.
