...
- The re-record carve-out is half implemented. A *total* re-record qualifies and a reissue or remaster excludes itself, both because of where MusicBrainz puts them. A **covers album** and a **partial re-record** are each their own release group with their own date, and MusicBrainz has no secondary type for either, so both pass as new albums.
- **Another band's version is not excluded either.** An earlier draft of this decision claimed the match rule handled it; that was wrong. The match rule stops a candidate binding to the *original* band's release group, but a source listing `Cover Band — Master of Puppets` matches Cover Band's own release group, primary type Album, and passes. Nothing refuses it.
