...
These three were **written by hand, not recorded from the live sources**: the
sandbox this project was built in denies outbound network access, so the pages
could not be fetched to record them. They reproduce each source's listing shape —
...
`npm run smoke:sources` is what checks them against reality, and until it has
been run against a live network nothing here has seen the real markup. When it
disagrees with a fixture, replace the fixture with the real body.
