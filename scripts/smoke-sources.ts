...
for (const sourceId of Object.keys(SOURCES) as SourceId[]) {
  ...
  const result = await fetchSource(ports, sourceId)
  spent += estimateCost(result.usage)

  console.log(`\n── ${sourceId} ───────────────────────────────`)
  console.log(`${result.url} → HTTP ${result.status} in ${Date.now() - started}ms`)
  console.log(
    `${result.rawBody.length} bytes raw, ${result.cleanedText.length} cleaned${
      result.truncated ? ' (truncated)' : ''
    }`,
  )
  console.log(`${result.candidates.length} candidates, ${result.droppedRows} rows dropped`)
