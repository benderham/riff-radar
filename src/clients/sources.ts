...
import { extractionSchema, normaliseCandidate } from '../domain/candidates.ts'
...
export const fetchSource = async (ports: Ports, sourceId: SourceId): Promise<SourceFetch> => {
  ...
  return {
    ...read,
    candidates,
    droppedRows: result.data.candidates.length - candidates.length,
    ...
    ...(candidates.length === 0
      ? { warning: `${url} yielded no candidates; the page may have changed shape` }
      : {}),
  }
}
